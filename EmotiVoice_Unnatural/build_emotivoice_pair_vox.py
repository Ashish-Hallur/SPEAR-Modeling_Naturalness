...
def aggregate_age_gender_for_speaker(
    audio_cache,
    wav_path,
    age_sex_model,
    device,
...
def aggregate_avd_for_segment(
    ...
    assert (MIN_SEGMENT_SEC - EPS) <= duration <= (MAX_SEGMENT_SEC + EPS), duration
    assert (VOX_MIN_SEC - EPS) <= duration <= (VOX_MAX_SEC + EPS), duration
...
def add_vox_features(
    ...
    for wav_path in [row["source_wav_path_prompt"], row["source_wav_path_response"]]:
        if wav_path not in speaker_cache:
            speaker_cache[wav_path] = aggregate_age_gender_for_speaker(
                audio_cache,
                wav_path,
                age_sex_model,
                device,
    ...
    prompt_spk = speaker_cache[row["source_wav_path_prompt"]]
    response_spk = speaker_cache[row["source_wav_path_response"]]
...
def main():
    ...
    with open(output_csv, "w", newline="", encoding="utf-8") as f:
        ...
        for row_dir in tqdm(row_dirs, desc=f"emotivoice vox shard {args.shard_idx}/{args.num_shards}"):
