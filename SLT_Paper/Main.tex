...
Building on prior reference-based prosody and rhythm evaluation for spoken dialogue systems \cite{DISTRIB}, we replace static matched ranges with learned conditional distributions over naturalness-relevant response metrics.
We train history-aware CVAE models on natural Seamless Interaction examples and evaluate them on held-out natural conversations and a TRACE-derived EmotiVoice probe, where TRACE is a temporal relationship-aware conversational entrainment study \cite{TRACE}.
The experiments show a tradeoff between accurately predicting natural response measurements and separating natural responses from synthetic unnatural probes.
Interpretable metric and context features are strongest for held-out natural prediction, while learned speech embeddings provide complementary signal for natural-vs-probe separation.
A listener study further supports the EmotiVoice condition as a perceptually less natural external probe.
\end{abstract}
...
\subsection{Conditional Modeling and Speech Representations}
A single preceding turn can allow many natural responses, so probabilistic conditional models are a natural fit for response-behavior evaluation.
Conditional variational autoencoders (CVAEs) are a standard latent-variable approach for modeling output variability under observed context \cite{kingma2013vae, sohn2015cvae}.
In parallel, learned speech representations have become useful for capturing non-semantic acoustic and paralinguistic information that may not be explicit in manually defined measurements.
Relevant examples include Trillsson embeddings for non-semantic speech representation \cite{shor2022trillsson}, Whisper representations for robust acoustic-language modeling \cite{radford2023whisper}, and VoxProfile features for speaker and affective speech analysis \cite{feng2025vox}.
...
We therefore use a CVAE to model residual response variability with a latent variable while conditioning on the current turn and recent interaction history.
We compare interpretable-only and embedding-augmented conditions because the former are easier to diagnose, while learned speech embeddings may retain acoustic or paralinguistic information that is not captured by the 12 response metrics.
