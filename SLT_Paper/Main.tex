...
We present a conversation-context conditioned evaluator that learns expected distributions over response-side speech measurements from natural dyadic interaction.
The method combines interpretable speech and context features, optional learned speech embeddings, recent conversational history, and a history-aware CVAE over naturalness-relevant response metrics.
We evaluate the framework on held-out natural conversations and on an external EmotiVoice-derived unnaturalness probe, using both automatic likelihood-based separation and human perceptual validation.
The experiments show that directly measured conversation features are strongest for predicting natural response measurements, while learned speech embeddings provide complementary signal for separating natural responses from synthetic unnatural probes.
These findings support a two-axis view of S2S naturalness evaluation: accurate modeling of natural conversational behavior and sensitivity to perceptually unnatural speech.
\end{abstract}
...
TRACE-derived probe & EmotiVoice-based synthetic or voice-converted dyadic clips from a temporal relationship-aware conversational entrainment study, reused here as an external unnaturalness stress test. \\
\bottomrule
\end{tabular}
\end{table}
...
To test whether models trained on natural responses assign lower likelihood to unnatural speech, we use EmotiVoice-derived conditions from TRACE, a temporal relationship-aware conversational entrainment study, as an external probe \cite{TRACE}.
...
The response space reported in this paper contains 12 naturalness-relevant measurements adapted from reference-based prosody and rhythm evaluation for spoken dialogue systems and extended with dimensional affect estimates \cite{DISTRIB}.
...
\begin{table*}[t]
\centering
\caption{Twelve-target response space used for the reported results. Segment-level means that each value is computed over the retained response segment.}
...
The model needs to represent a range of plausible response behaviors rather than a single canonical response, because many timing, prosodic, and affective patterns can be natural in the same conversation context.
...
\caption{Two-lane study design. Natural Seamless conversations provide the training and held-out natural evaluation examples, while the TRACE-derived EmotiVoice probe passes through the same adjacent-turn and measurement pipeline as an external unnaturalness stress test. The shared feature representation combines interpretable context features, optional conditioning-turn embeddings, 12 response targets, and a history window before the history-aware CVAE is evaluated on prediction and separation axes.}
\label{fig:pipeline}
\end{figure*}
...
\section{Results}
\subsection{Twelve-Target Model Sweep}
Table~\ref{tab:clean_sweep} reports all aggregate results over the 12 naturalness targets in Table~\ref{tab:target_metrics}.
The best natural-response predictor is the interpretable-only model with five previous turns, with MSE 0.025 and $R^2=0.975$.
...
\caption{Two views of the prediction-separation tradeoff. Panel A plots natural-test MSE against natural-minus-probe likelihood gap for all 16 conditions, labeling only the key models. Panel B compares normalized prediction and separation ranks for the best predictor, strongest separator, and balanced-tier models.}
\label{fig:tradeoff}
\end{figure*}
...
\begin{table*}[t]
\centering
\caption{High-signal per-feature findings over the 12-target response space. Prediction winners use natural-test MSE; separation winners use per-feature $\Delta$LL.}
...
The automatic separation results are strengthened by the listening study reported with TRACE over mixed two-speaker clips \cite{TRACE}.
...
A deployable evaluator must score candidate responses using only the conditioning turn and history, without conditioning the latent variable on the response being evaluated.
Second, the TRACE-derived EmotiVoice probe is perceptually validated, but it represents one family of unnatural speech manipulations from a recent preprint, not the full space of S2S failure modes.
...
Finally, the target space is intentionally speech-native and does not evaluate semantic appropriateness, task success, factuality, or user satisfaction.
...
The paper extends reference-based prosody and rhythm evaluation for spoken dialogue systems from static matched regimes to learned conditional response distributions, and uses TRACE-derived EmotiVoice examples as an external unnaturalness probe \cite{DISTRIB,TRACE}.
...
Future work will convert the posterior fit scores into a prior-predictive evaluator and directly validate per-item scores against listener judgments.
