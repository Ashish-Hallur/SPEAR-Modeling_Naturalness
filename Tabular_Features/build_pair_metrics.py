...
def build_pair_rows_for_dyad(
    ...
    for i in range(1, len(all_events)):
        ...
        prompt_f0 = compute_segment_f0(sound_cache, prev_ev["wav_path"], prompt_start, prompt_end)
        response_f0 = compute_segment_f0(sound_cache, cur_ev["wav_path"], response_start, response_end)

        prompt_temp = compute_local_temporal(prompt_words, prompt_start, prompt_end)
        response_temp = compute_local_temporal(response_words, response_start, response_end)
...
def main():
    ...
    fieldnames = [
        ...
        "status_pair", "status_prompt_prosody", "status_response_prosody",
    ...
    with open(output_train_csv, "w", newline="") as train_f, open(output_test_csv, "w", newline="") as test_f:
        ...
        for row in tqdm(shard_rows, desc=f"metrics shard {args.shard_idx}/{args.num_shards}"):
